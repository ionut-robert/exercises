...
def opencsv():
    ...
    return ids,name,salary,data,header

ids,name,salary,data,header = opencsv()

def InfoUpload(employee_name):
    idvar.set(next((row[0] for row in data if row[1]==employee_name),''))
    NameVar.set(next((row[1] for row in data if row[1]==employee_name),''))
    SalaryVar.set(next((row[2] for row in data if row[1]==employee_name),''))
...
employee_spnbox = OptionMenu(win,var,*name,command=InfoUpload)
